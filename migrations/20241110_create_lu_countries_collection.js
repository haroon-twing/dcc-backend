...
async function up() {
  try {
    ...
    const collectionExists = await mongoose.connection.db.listCollections({ name: 'lu_countries' }).hasNext();
    
    if (!collectionExists) {
      // Create collection with schema
      await mongoose.connection.db.createCollection('lu_countries');
      console.log('Created lu_countries collection');
      
      // Insert countries
      await LuCountry.insertMany(countries);
      console.log(`✓ Added ${countries.length} countries to lu_countries collection`);
    } else {
      console.log('lu_countries collection already exists');
    }
...
async function down() {
  try {
    ...
    // Drop the collection
    await mongoose.connection.db.dropCollection('lu_countries');
    console.log('✓ lu_countries collection dropped');
